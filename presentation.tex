...
\begin{frame}{Kontekst doktorske disertacije}
  \centering
...
\begin{frame}{Motivacija}
  ...
  \note[item]{Ključno je, da s tem ko dvignemo kompleksnost, zadeva ne postane neuporabno počasna ali pa pa so tabele prevelike.}
\end{frame}
...
\begin{frame}{Stanje tehnike}
  ...
  \begin{tikzpicture}[ampersand replacement=\!]
    ...
    \matrix (m) [column sep=-0.6cm, row sep=0.2cm] {
      ...
      \node[dot] (nscanner) {}; \! \node[dot] (gcascanner) {}; \\[0.7cm]

      \node[edot, label=right:--\ razčlenjevalnik] (lparser) {}; \! \node[dot, label=right:--\ pregledovalnik] (lscanner) {};\\
    };
  ...
  \note[item]{Nismo prvi, ki smo poskušali rešiti ta problem. Predlaganih je bilo že nekaj arhitektur. Veliki krožec je tukaj razčlenjevalnik, mali krožec pa pregledovalnik. Ti diagrami prikazujejo komunikacijo med njima.}
...
\begin{frame}{Stanje tehnike}
  ...
  \begin{tikzpicture}
    ...
    \draw (nawrocki) edge[->, bend right=50] (contextaware);
...
\begin{frame}{Predlagana arhitektura RNGSGLR: Moč}
  \centering
  \begin{tikzpicture}[ampersand replacement=\!]
    \tikzset{
      ->,
  ...
  \note[item]{Programski jeziki, pa so na splošno neomejeni.}
...
\begin{frame}{Predlagana arhitektura RNGSGLR: Delovanje}
  \begin{itemize}
    \item Ena rekurzivna mreža prehodov (RTN)
    \item Filtriranje glede na levi in desni kontekst
    \item Delni vpogled v naprej (ohranjanje sinhronizacije)
  \end{itemize}
  ...
  \note[item]{Da se ohrani sinhronizacija procesov razčlenjevalnika in pregledovalnika uporabljamo delen vpogled naprej}
\end{frame}

\begin{frame}{Predlagana arhitektura RNGSGLR: Dodatne funkcionalnosti}
  \begin{itemize}
...
\begin{frame}{Predlagana arhitektura RNGSGLR: Prednosti}
  \begin{itemize}
    \item Vgrajeni podjeziki (brez označevalcev)
    \item Integrirana uniformna specifikacija
    \item Kompozicija specifikacij
    \item Lažji razvoj specifikacij
    \item Ohranjanje leksikalne strukture
    \item Ekspresivna leksikalna sintaksa
  \end{itemize}

  \note[item]{Leksikalna sintaksa se lahko razlikuje v različnih delih programskega jezika.}
  \note[item]{Lahko se spreminja prosto, označevalci za spremembo leksikalne sintakse niso potrebni.}
  \note[item]{Leksikalna in kontekstno-neodvisna sintaksa sta opisani z enim formalizmom.}
  \note[item]{Ker so specifikacije zaprte pod unijo, konkatenacijo in Kleene zvezdico je mogoča kompozicija več specifikacij.}
  \note[item]{Pri razvoju, lahko dodajamo poljubne produkcije in regularne definicije in nikoli ne pride do napake, saj naša arhitektura sprejme vse specifikacije.}
  \note[item]{Pisec gramatike lahko doseže poljubno granulacijo tako, da namesto leksikalne sintakse uporabit kontekstno neodvisno.}
  \note[item]{Leksiklana sintaksa podpira razširjene regularne izraze, tako da je na voljo tudi presek, razlika in komplement.}
  \note[item]{Naša arhitektura ima vse prednosti razčlenjevanja brez pregledovalnika, in hkrati boljšo učinkovitost.}
\end{frame}

\begin{frame}{Eksperiment}
  \begin{itemize}
    \item Primerjava učinkovitosti z RNGLR v kombinaciji z kontekstno zavednim pregledovanjem in RNSGLR
...
\begin{frame}{Eksperiment: Primerjava RNGLR -- RNGSGLR}
...
\begin{tikzpicture}
  ...
  \begin{axis}[
    ...
    xmin=0, xmax=1000,
    ymin=0, ymax=20,
    ylabel={Čas $[\si{\second}]$},
    xlabel={Velikost $[\si{\kilo\byte}]$},
...
\begin{frame}{Eksperiment: Primerjava RNSGLR -- RNGSGLR}
...
\begin{frame}{Teza}
  ...
  \note[item]{To je teza, ki jo bomo potrdili z naslednjimi hipotezami.}
\end{frame}
...
\begin{frame}{Hipoteza 1}
   ...
   \begin{scriptsize}
     Razpoznavalnik je pravilen za dano specifikacijo $\Spec$, če sprejme vhoden niz $\omega$, če in samo če $\omega \in \Language{(\Spec)}$ in ga drugače zavrne.
     Najprej pokažemo, če naš algoritem sprejme $\omega$ potem $\omega \in \Language{(\Spec)}$.
     Nato pokažemo, če $\omega \in \Language{(\Spec)}$ potem naš algoritem sprejme $\omega$.
     To naredimo tako, da pokažemo, da naš algoritem sprejme $\omega$ natanko tedaj, ko črkovni RNGLR razpoznavalnik za gramatike z regularnimi desnimi stranmi sprejme $\omega$.
   \end{scriptsize}
...
\begin{frame}{Hipoteza 2}
  ...
  \begin{scriptsize}
    Pokažemo eksperimentalno, predlagana arhitektura je 72\%, 38\%, 57\%, 64\%, 47\% slabša za JSON, Pascal, C, COBOL, Fortran.
  \end{scriptsize}
...
\begin{frame}{Hipoteza 3}
  ...
  \begin{scriptsize}
    Velikost tabel je primerljiva s kontekstno zavednim pregledovanjem, kjer izgradimo avtomat za vsako stanje.
  \end{scriptsize}
...
\begin{frame}{Hipoteza 4}
  ...
  \begin{scriptsize}
    Pokažemo eksperimentalno, čas procesiranja za JSON, Pascal, C, COBOL, Fortran, je linearno odvisen od dolžine vhodnega niza.
  \end{scriptsize}
...
\begin{frame}{Hipoteza 5}
  ...
  \begin{scriptsize}
    Časovna kompleksnost razčlenjevalnika je v veliki meri pogojena s številom korakov, ki so potrebni, da se poiščejo poti med procesiranjem redukcij.
    Časovna kompleksnost pregledovalnika je pogojena z številom robov, ki jih lahko ima vsako vozlišče ustvarjeno z branjem znaka.
  \end{scriptsize}
...
\begin{frame}{Objave}
  ...
  \\
  \begin{tiny}
    {Žiga Leber, Matej Črepinšek, Marjan Mernik, Tomaž Kosar}
  \end{tiny}
  \\
  \begin{tiny}
    {Mathematics 2022, 10(14), 2436}
  \end{tiny}
  \par
  \begin{scriptsize}
  ...
  \\
  \begin{tiny}
    {Žiga Leber, Matej Črepinšek, Tomaž Kosar}
  \end{tiny}
  \\
  \begin{tiny}
    {VL/HCC 2019}
  ...
  \\
  \begin{tiny}
    {Tomaž Kosar, Srđa Bjeladinović, Dragana Ostojić, Milica S. Škembarević, Žiga Leber, Olga A. Jejić, Filip Furtula, Miloš D. Ljubisavljević, Ivan S. Luković, Marjan Mernik}
  \end{tiny}
  \\
  \begin{tiny}
    {FedCSIS 2024}
