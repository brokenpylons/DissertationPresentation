...
\title{\large Generalization of the Context-Aware Scanning Architecture for All Character-Level Context-Free Languages}
\date{\today}
\author{Žiga Leber}
\institute{Programming methodology laboratory}
...
\begin{frame}{Idea}
  ...
  \end{LARGE}
  \note[item]{That means, we have a unified algorithm like in a scannerless approach, one automaton, one set of tables, one data structure for parsing, but still conceptually using a scanner.}
  \note[item]{We deconstruct scanner based parsing into its constituent parts, and reassemble them into a generalized algorithm.}
  \note[item]{Our algoritm generalizes scanning in the same way GLR generalizes LR.}
  \note[item]{The main insight is that there is interpretation change at the boundary between the parser and the scanner, and a limited form of non-canonical parsing is used.}
  \note[item]{In non-canonical parsing the parsing decision can be delayed and reductions can be performed on the rest of the input, then the reduced non-terminals are used as lookahead.}
\end{frame}
...
\begin{frame}{Construction}
  ...
  \note[item]{The central formalism used is the recursive transition network (RTN).}
  \note[item]{The construction is the same as non-deterministic LR construction in Parsing Techniques.}
  \note[item]{The difference is that we use Brzozowski derivatives, instead of DeRemer derivatives, that way intersection, difference and complement are supported as well.}
  \note[item]{Then the optimizations are applied.}
...
\begin{frame}{Regular right part grammar}
  \begin{equation*}
    \begin{aligned}
      \_\LblSep\NT{S}' &\Arrow_0 \NT{S} \Spc \KleenePlus{\EOF}\\
      \_\LblSep\NT{S} &\Arrow_1 \NT{A} \Union \T{b} \Spc \Kleene{(\T{c} \Spc \T{b})}\\
      \_\LblSep\NT{A} &\Arrow_2 \T{b} \Spc \T{c}
    \end{aligned}
    \hspace{3em}
    \begin{aligned}
      \_\LblSep\T{b} &\Arrow_3 \Term{x}\\
      \_\LblSep\T{c} &\Arrow_4 \Term{y}\\
      {}
    \end{aligned}
  \end{equation*}

  \note[item]{The right hand sides are regular expressions.}
  \note[item]{The grammar is augmented with a start production.}
...
\begin{frame}{Unoptimized RTN}
  ...
  \note[item]{Connecting to an item for that non-terminal.}
\end{frame}
...
\begin{frame}{Optimized RTN}
  ...
  \note[item]{Limited form of automaton driven early construction, the kernel items and non-kernel items are separated by a transition.}
\end{frame}
...
\begin{frame}{Enhanced RTN}
  ...
  \note[item]{The edges of the enhanced RTN are labeled with enhanced symbols.}
...
\begin{frame}{Extracted enhanced productions}
  ...
  \begin{tikzpicture}[tips=proper, node distance=1.4cm and 1.5cm, initial text=]
    \begin{scope}
      \node [pstate, label={above left:$(0, \NT{S}) \Arrow_1$}] (0) {$0\quad 3$};
    ...
    \begin{scope}[shift={(0, -4)}]
      \node [pstate, label={above left:$(0, \NT{A}) \Arrow_2$}] (0) {$0\quad 11$};
    ...
    \begin{scope}[shift={(0, -6)}]
      \node [pstate, label={above left:$(0, \T{b})\ (5, \T{b})\ (7, \T{b}) \Arrow_3$}] (0) {$8\quad 6$};
      \node [pstate, right=of 0] (1) {$9\quad 7$};

      \draw(0) edge[->] node[auto]{$(8, \Term{x})$} (1);
    \end{scope}
    \begin{scope}[shift={(0, -8)}]
      \node [pstate, label={above left:$(4, \T{c})\ (6, \T{c}) \Arrow_4$}] (0) {$10\quad 8$};
  ...
  \note[item]{The reduce lookahead is computed, by first computing lookback and then FOLLOW of the left-hand-side symbol.}
  \note[item]{There is a lookback if the right-hand-side of the enhanced production includes the given enhanced state.}
  \note[item]{In enhanced state (6, 4) the lookahead is computed as FOLLOW(0, S).}
\end{frame}
...
\begin{frame}{Non-canonical RTN}
  ...
  \note[item]{In general in each reduce state, a call transition is added for each lookahead enhanced symbol.}
  \note[item]{This is a special case of non-canonical construction.}
